...
export default class SignInScreen extends React.Component {
  ...
  handleUberLogin = async () => {
    ...
    let results = await AuthSession.startAsync({
      authUrl: `https://login.uber.com/oauth/v2/authorize?response_type=code&client_id=jloIvMctI2Pnd3lbrKNpqpDiSfJD8SPk&scope=profile+ride_widgets+request+request_receipt&redirect_uri=${encodeURIComponent(
        redirectUrl
      )}`,
    })
    if (results.type !== 'success') {
      ...
      axios({
        ...
        params: {
          ...
          scope: 'profile+ride_widgets+request+request_receipt',
        },
      })
        .then(response => {
